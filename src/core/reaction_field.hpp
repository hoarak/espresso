...
inline void add_rf_coulomb_pair_force_no_cutoff(const Particle * const p1, const Particle * const p2, double d[3], double dist, double force[3])
{
   ...
   ONEPART_TRACE(if(p1->id()==check_id) fprintf(stderr,"%d: OPT: RF   f = (%.3e,%.3e,%.3e) with part id=%d at dist %f fac %.3e\n",this_node,p1->f.f[0],p1->f.f[1],p1->f.f[2],p2->id(),dist,fac));
   ONEPART_TRACE(if(p2->id()==check_id) fprintf(stderr,"%d: OPT: RF   f = (%.3e,%.3e,%.3e) with part id=%d at dist %f fac %.3e\n",this_node,p2->f.f[0],p2->f.f[1],p2->f.f[2],p1->id(),dist,fac));
}
				...
				double d[3], double dist, double force[3])
{
  ...
  ONEPART_TRACE(if(p1->id()==check_id) fprintf(stderr,"%d: OPT: INTER_RF   f = (%.3e,%.3e,%.3e) with part id=%d at dist %f fac %.3e\n",this_node,p1->f.f[0],p1->f.f[1],p1->f.f[2],p2->id(),dist,fac));
  ONEPART_TRACE(if(p2->id()==check_id) fprintf(stderr,"%d: OPT: INTER_RF   f = (%.3e,%.3e,%.3e) with part id=%d at dist %f fac %.3e\n",this_node,p2->f.f[0],p2->f.f[1],p2->f.f[2],p1->id(),dist,fac));
}
