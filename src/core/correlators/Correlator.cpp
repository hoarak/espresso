...
int Correlator::get_data() {
  ...
  {
    auto const A_val = A_obs->calculate();
    memmove(A[0][newest[0]], A_val.data(), dim_A * sizeof(double));
  }

  if (!autocorrelation) {
    auto const B_val = B_obs->calculate();
    memmove(B[0][newest[0]], B_val.data(), dim_B * sizeof(double));
  }
...
int Correlator::read_data_from_file(const char *filename, bool binary) {
  ...
  for (unsigned int i = 0; i < hierarchy_depth; i++) {
    for (unsigned int j = 0; j < tau_lin + 1; j++) {
      if (read_double(fp, A[i][j], dim_A, binary))
        return 1;
    }
  }
  if (!autocorrelation) {
    for (unsigned int i = 0; i < hierarchy_depth; i++) {
      for (unsigned int j = 0; j < tau_lin + 1; j++) {
        if (read_double(fp, B[i][j], dim_B, binary))
          return 1;
      }
    }
  }
  for (unsigned int i = 0; i < n_result; i++) {
    if (read_double(fp, result[i], dim_corr, binary))
      return 1;
  }
  if (read_uint(fp, n_sweeps, n_result, binary))
    return 1;
  if (read_uint(fp, n_vals, hierarchy_depth, binary))
    return 1;
  if (read_uint(fp, newest, hierarchy_depth, binary))
    return 1;

  if (read_double(fp, A_accumulated_average, dim_A, binary))
    return 1;
  if (read_double(fp, A_accumulated_variance, dim_A, binary))
    return 1;
  if (!autocorrelation) {
    if (read_double(fp, B_accumulated_average, dim_B, binary))
      return 1;
    if (read_double(fp, B_accumulated_variance, dim_B, binary))
      return 1;
  }
  if (read_uint(fp, &(n_data), 1, binary))
    return 1;
  if (read_uint(fp, &(t), 1, binary))
    return 1;
  if (read_double(fp, &(last_update), 1, binary))
    return 1;
  fclose(fp);
  return 0;
}
...
int scalar_product(double *A, unsigned int dim_A, double *B, unsigned int dim_B,
                   double *C, unsigned int dim_corr, void *args) {
  double temp = 0;
  unsigned int i;
...
int complex_conjugate_product(double *A, unsigned int dim_A, double *B,
                              ...
                              unsigned int dim_corr, void *args) {
  unsigned int i, j;
  if (!(dim_A == dim_B)) {
