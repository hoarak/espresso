 ...
 *  bonds --- parameter: [N, a_i, b_i, c_i], function: U(bond)  = sum_(i=1,N)
 * {a_i*exp[-(bond-b_i)^2 /(c_i^2)]}.
 *  angles--- parameter: [N, a_i, b_i, c_i], function: U(cos(angl)) =
 * sum_(i=1,N) {a_i*exp[-(cos(angl)-b_i)^2 / (2 * c_i^2)]}.
 *  dihedrals---parameter: [N, a_i, b_i, c_i], function: U(dihe)= sum_(i=1,N)
 * {a_i*(1+Math.cos(c_i*dihe+b_i))}.
 *  Require feature OVERLAPPED compiled in myconfig.hpp (for more info of
 * FEATURES, see \ref config.hpp ).
*/
...
int overlapped_bonded_set_params(int bond_type,
                                 OverlappedBondedInteraction overlap_type,
                                 char *filename);
...
*/
inline int calc_overlap_bond_force(Particle *p1, Particle *p2,
  ...
  for (ig = 0; ig < iaparams->p.overlap.noverlaps; ig++) {
    ev = (dist - iaparams->p.overlap.para_b[ig]) /
         iaparams->p.overlap.para_c[ig];
    prob = prob + iaparams->p.overlap.para_a[ig] * exp(-1.0 * ev * ev);
    Apart =
        Apart +
        iaparams->p.overlap.para_a[ig] * exp(-1.0 * ev * ev) *
            (dist - iaparams->p.overlap.para_b[ig]) /
            (iaparams->p.overlap.para_c[ig] * iaparams->p.overlap.para_c[ig]);
  }
  ...
  ONEPART_TRACE(if (p1->id() == check_id)
                    fprintf(stderr, "%d: OPT: HARMONIC f = (%.3e,%.3e,%.3e) "
                                    "with part id=%d at dist %f fac %.3e\n",
                            this_node, p1->f()[0], p1->f()[1], p1->f()[2],
                            p2->id(), dist2, fac));
  ONEPART_TRACE(if (p2->id() == check_id)
                    fprintf(stderr, "%d: OPT: HARMONIC f = (%.3e,%.3e,%.3e) "
                                    "with part id=%d at dist %f fac %.3e\n",
                            this_node, p2->f()[0], p2->f()[1], p2->f()[2],
                            p1->id(), dist2, fac));
...
/** Computes the two body overlapped angle interaction energy (see \ref
   ...
   tclcommand_inter).
    ...
    Needs feature OVERLAPPED compiled in (see \ref config.hpp).
*/
inline int overlap_bond_energy(Particle *p1, Particle *p2,
                               Bonded_ia_parameters *iaparams, double dx[3],
                               double *_energy) {
...
inline int calc_overlap_angle_force(Particle *p_mid, Particle *p_left,
  ...
  for (ig = 0; ig < iaparams->p.overlap.noverlaps; ig++) {
    ev = (-cosine - iaparams->p.overlap.para_b[ig]) /
         iaparams->p.overlap.para_c[ig];
    prob = prob + iaparams->p.overlap.para_a[ig] * exp(-1.0 * ev * ev);
    Apart =
        Apart +
        iaparams->p.overlap.para_a[ig] * exp(-1.0 * ev * ev) *
            (-cosine - iaparams->p.overlap.para_b[ig]) /
            (iaparams->p.overlap.para_c[ig] * iaparams->p.overlap.para_c[ig]);
  }
  fac = -2. * (Apart / prob);
...
/** Computes the four body overlapped dihedral interaction force.
   ...
   tclcommand_inter).
    ...
    Needs feature OVERLAPPED compiled in (see \ref config.hpp).
*/
inline int calc_overlap_dihedral_force(Particle *p2, Particle *p1, Particle *p3,
                                       Particle *p4,
                                       Bonded_ia_parameters *iaparams,
  ...
  for (ig = 0; ig < iaparams->p.overlap.noverlaps; ig++) {
    ...
    if (fabs(sin(phi)) < TINY_SIN_VALUE) {
      ...
      sinmphi_sinphi = iaparams->p.overlap.para_c[ig] *
                       cos(iaparams->p.overlap.para_c[ig] * phi +
                           iaparams->p.overlap.para_b[ig]) /
                       cosphi;
    } else {
      sinmphi_sinphi = sin(iaparams->p.overlap.para_c[ig] * phi -
                           iaparams->p.overlap.para_b[ig]) /
                       sin(phi);
    }
    fac *= sinmphi_sinphi;
...
/** Computes the four body overlapped dihedral interaction energy (see \ref
   ...
   tclcommand_inter).
    ...
    Needs feature OVERLAPPED compiled in (see \ref config.hpp).
*/
inline int overlap_dihedral_energy(Particle *p2, Particle *p1, Particle *p3,
  ...
  for (ig = 0; ig < iaparams->p.overlap.noverlaps; ig++) {
    fac = cos(iaparams->p.overlap.para_c[ig] * phi +
              iaparams->p.overlap.para_b[ig]);
    fac += 1.0;
