 ...
 */
static void dp3m_calc_local_ca_mesh(P3MParameters &params, double skin);
...
double dp3m_calc_kspace_forces(int force_flag, int energy_flag) {
  ...
  if (dp3m.sum_mu2 > 0) {
    ...
    dp3m.sm.gather(dp3m.rs_mesh_dip, Utils::MemoryOrder::ROW_MAJOR);
  ...
  if (force_flag) {
    ...
    if (dp3m.sum_mu2 > 0) {
      ...
      for (d = 0; d < 3; d++) {
        ...
        dp3m.sm.spread(dp3m.rs_mesh, Utils::MemoryOrder::ROW_MAJOR);
