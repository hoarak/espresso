      ...
      *(int *)insert = np;
      ...
      for (int p = 0; p < np; p++) {
        ...
        if (data_parts & GHOSTTRANS_POSSHFTD) {
          static_assert(std::is_pod<ParticlePosition>::value, "");
          ParticlePosition *pp = new (insert) ParticlePosition(*pt);
          ...
          static_assert(std::is_pod<ParticlePosition>::value, "");
          new (insert) ParticlePosition(*pt);
  ...
  for (int pl = 0; pl < gc->n_part_lists; pl++) {
    ...
    if (data_parts & GHOSTTRANS_PARTNUM) {
      ...
      for (int p = 0; p < np; p++) {
        ...
        if (data_parts & GHOSTTRANS_POSITION) {
          *pt = *(reinterpret_cast<ParticlePosition *>(retrieve));
          retrieve += sizeof(ParticlePosition);
#ifdef LEES_EDWARDS
          /* special wrapping conditions for x component of y LE shift */
          if (gc->shift[1] != 0.0) {
            /* LE transforms are wrapped
               ---Using this method because its a shortcut to getting a
               neat-looking verlet list. */
            if (pt->pos()[0] -
                    (my_left[0] + cur_list->myIndex[0] * dd.cell_size[0]) >
                2 * dd.cell_size[0])
              pt->pos()[0] -= box_l[0];
            if (pt->pos()[0] -
                    (my_left[0] + cur_list->myIndex[0] * dd.cell_size[0]) <
  ...
  for (pl = 0; pl < offset; pl++) {
    ...
    if (data_parts & GHOSTTRANS_PARTNUM) {
      ...
      for (p = 0; p < np; p++) {
        ...
        if (data_parts & GHOSTTRANS_POSSHFTD) {
          *pt2 = static_cast<ParticlePosition>(*pt1);
          for (int i = 0; i < 3; i++)
          ...
          if (gc->shift[1] != 0.0) {
            /* LE transforms are wrapped */
            if (pt2->pos()[0] -
                    (my_left[0] + dst_list->myIndex[0] * dd.cell_size[0]) >
                2 * dd.cell_size[0])
              pt2->pos()[0] -= box_l[0];
            if (pt2->pos()[0] -
                    (my_left[0] + dst_list->myIndex[0] * dd.cell_size[0]) <
    ...
    if (comm_type == GHOST_LOCL)
      ...
      case GHOST_RDCE:
        GHOST_TRACE(fprintf(stderr, "%d: ghost_comm reduce to %d (%d bytes)\n",
                            this_node, node, n_s_buffer));
        if (node == this_node)
          MPI_Reduce(s_buffer, r_buffer, n_s_buffer, MPI_BYTE, MPI_FORCES_SUM,
                     node, comm_cart);
        else
          MPI_Reduce(s_buffer, NULL, n_s_buffer, MPI_BYTE, MPI_FORCES_SUM, node,
                     comm_cart);
        break;
      }
...
void ghost_init() { MPI_Op_create(reduce_forces_sum, 1, &MPI_FORCES_SUM); }
