...
//  double dx,dy,dz;
  ...
  for ( i = 0; i<ids1->n; i++ ) {
    if (ids1->e[i] >= n_part)
      return 1;
    pos1[0]=partCfg[ids1->e[i]].pos()[0];
    pos1[1]=partCfg[ids1->e[i]].pos()[1];
    pos1[2]=partCfg[ids1->e[i]].pos()[2];
    for ( j = 0; j<ids2->n; j++ ) {
      if (ids2->e[j] >= n_part)
        return 1;
      if (ids2->e[j] == ids1->e[i]) // do not count self-interaction :-)
        continue;
      A[i] = 0;
      pos2[0]=partCfg[ids2->e[j]].pos()[0];
      pos2[1]=partCfg[ids2->e[j]].pos()[1];
      pos2[2]=partCfg[ids2->e[j]].pos()[2];
